...
export function localEnvironment(){const objects=fixtureObjects();return{_fixtureObjects:objects,DB:new D1,R2_BUCKET:{get:key=>Promise.resolve(objects.has(key)?new R2Object(objects.get(key)):null)},LOCAL_FIXTURE:"1",ASSETS:{fetch:()=>new Response("not found",{status:404})}}}
