...
def main():
    ...
    for sid,events in sorted(bysec.items()):
        ...
        rec={"security_id":sid,"signal_date":asof.date().isoformat(),"close":close,"event_count":len(events),"events":[]}
        for p,xs in sorted(events):
            ...
            rec["events"].append({"pivot_level":p,"distance_above_pivot_pct":(close/p-1)*100,
