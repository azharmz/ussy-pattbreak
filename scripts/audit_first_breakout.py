...
def main():
    ...
    payload={"schema":"first-breakout-audit-v1","as_of_date":asof.date().isoformat(),"volume_threshold":VOL_THRESHOLD,
      ...
      "security_summary":sorted(sec_summary,key=lambda z:(-z["assessment_candidates"],z["security_id"])),"rows":rows}
    out=Path(a.output); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
    print(json.dumps({k:payload[k] for k in ["as_of_date","candidate_assessments","classification_counts","unique_securities","unique_security_pivots"]},sort_keys=True))
